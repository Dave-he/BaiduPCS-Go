...
- [BaiduPCS-Go 百度网盘客户端(加强版)](#baidupcs-go-百度网盘客户端加强版)
  - [注意](#注意)
  - [目录](#目录)
- [特色](#特色)
- [版本更新](#版本更新)
      - [该版本存在已知问题将导致上传文件失败及出现空文件，建议跳过更新](#该版本存在已知问题将导致上传文件失败及出现空文件建议跳过更新)
- [编译/交叉编译 说明](#编译交叉编译-说明)
  - [Linux/Darwin 例子: 编译 Windows 下的 64 位程序](#linuxdarwin-例子-编译-windows-下的-64-位程序)
  - [Windows 例子: 编译 Linux 下的 32 位程序](#windows-例子-编译-linux-下的-32-位程序)
- [下载/运行 说明](#下载运行-说明)
  - [安装](#安装)
  - [Windows](#windows)
...
- [命令列表及说明](#命令列表及说明)
  ...
  - [登录百度帐号](#登录百度帐号)
    - [常规登录百度帐号](#常规登录百度帐号)
    - [使用百度 BDUSS 和 百度网盘 STOKEN 来登录百度账号](#使用百度-bduss-和-百度网盘-stoken-来登录百度账号)
    - [使用百度 Cookies 来登录百度账号(推荐)](#使用百度-cookies-来登录百度账号推荐)
      - [例子](#例子)
  - [列出帐号列表](#列出帐号列表)
  - [获取当前帐号](#获取当前帐号)
  - [切换百度帐号](#切换百度帐号)
  ...
  - [获取网盘配额](#获取网盘配额)
  - [切换工作目录](#切换工作目录)
    - [切换工作目录后自动列出工作目录下的文件和目录](#切换工作目录后自动列出工作目录下的文件和目录)
      - [例子](#例子-1)
  - [输出工作目录](#输出工作目录)
  - [列出目录](#列出目录)
    - [可选参数](#可选参数)
...
- 基本修复了登录验证失效问题([#15](https://BaiduPCS-Go/issues/15))
...
│   ├── sdk/               # SDK相关功能
│   │   ├── sdk_auth.go    # 认证相关
│   │   ├── sdk_commands.go # CLI命令定义
│   │   └── sdk_search.go  # 搜索功能
│   ├── ui/                # 用户界面相关
│   │   ├── file_selector.go # 文件选择器
│   │   └── progress_bar.go   # 进度条
│   └── utils/             # 工具函数
│       └── utils.go       # 通用工具
├── pkg/                   # 公共包（预留）
