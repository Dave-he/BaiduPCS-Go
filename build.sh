...
IOSBuild() {
  ...
  $go build -ldflags "-X main.Version=$version -s -w" -o "armv7" BaiduPCS-Go
  jtool --sign --inplace --ent ../../entitlements.xml "armv7"
  export GOARCH=arm64
  $go build -ldflags "-X main.Version=$version -s -w" -o "arm64" BaiduPCS-Go
...
RicePack() {
  return # 已取消web功能
  rice -i BaiduPCS-Go/internal/pcsweb append --exec "$output/$1/$2"
}
